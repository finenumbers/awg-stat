...
- Периодически меряет задержку до каждого хоста по ICMP (если доступен)
...
У названия сервера три независимых слота. Опрос VPN (SSH): метка протокола (`AmneziaWG 3.1`) или на том же месте **«Ошибка опроса»**. ICMP (один echo на резолвленный IP в том же цикле): RTT в миллисекундах или на том же месте **«Сервер недоступен»**, даже если SSH-опрос успешен. Отдельный TCP connect на `:22` и история задержки не ведутся.

Третий слот — ограничение хоста в ТСПУ / реестре по [cheburcheck.ru](https://cheburcheck.ru). `gate-poller` раз в 10 минут отправляет **публичный** hostname или IP сервера на `https://cheburcheck.ru`. Приватные, CGNAT и localhost адреса не уходят. Пока вердикта нет, пилюля скрыта; дальше **«Не ограничен»** или **«Заблокирован»** в одной строке с версией: `AmneziaWG 3.1 / Не ограничен`. Клик открывает разбор на cheburcheck. Последний вердикт хранится в БД и не затирается, если cheburcheck недоступен. Отключить: `GATE_BLOCK_CHECK=0` у `gate-poller`. Новый Docker cap не нужен, нужен исходящий TCP/443 с poller.

На dashboard карточка красится светло-красной, если ICMP «Сервер недоступен», и светло-синей при блокировке ТСПУ. Если оба сразу — красный (нет пинга) важнее.

ICMP **не обязателен** для опроса VPN. В Docker нужен `cap_add: NET_RAW` у `gate-poller` (уже в compose/стеке). Если бинаря `ping` нет или нет права на raw socket, пробы отключаются до рестарта poller и последнее сохранённое значение не затирается. Если облако режет ICMP, бейдж будет красным, опрос VPN продолжается. Проверка:
