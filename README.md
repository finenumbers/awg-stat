...
Третий слот — ограничение хоста в ТСПУ / реестре по [cheburcheck.ru](https://cheburcheck.ru). `gate-poller` раз в 10 минут отправляет **публичный** hostname или IP сервера на `https://cheburcheck.ru`. Приватные, CGNAT и localhost адреса не уходят. Пока вердикта нет, пилюля скрыта; дальше **«Не ограничен»** или **«Заблокирован»** под пингом. Клик открывает разбор на cheburcheck. Последний вердикт хранится в БД и не затирается, если cheburcheck недоступен. Отключить: `GATE_BLOCK_CHECK=0` у `gate-poller`. Новый Docker cap не нужен, нужен исходящий TCP/443 с poller.
