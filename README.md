...
У названия сервера два независимых слота. Опрос VPN (SSH): метка протокола (`AmneziaWG 3.1`) или на том же месте **«Ошибка опроса»**. ICMP (один echo на резолвленный IP в том же цикле): RTT в миллисекундах или на том же месте **«Сервер недоступен»**, даже если SSH-опрос успешен. Отдельный TCP connect на `:22` и история задержки не ведутся.
