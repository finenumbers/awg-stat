...
Поллер меряет ICMP (один echo на резолвленный IP) в том же цикле, что и опрос VPN. Последнее успешное значение показывается под меткой протокола (`AmneziaWG 3.1`). Отдельный TCP connect на `:22` и история задержки не ведутся.

ICMP **не обязателен**. В Docker нужен `cap_add: NET_RAW` у `gate-poller` (уже в compose/стеке). Без capability или если облако режет ICMP, под меткой протокола задержки не будет, опрос VPN продолжается. Проверка:
