...
Поллер меряет ICMP (один echo на резолвленный IP) в том же цикле, что и опрос VPN. Под меткой протокола (`AmneziaWG 3.1`) всегда последний результат пробы: RTT в миллисекундах или **«Сервер недоступен»**, даже если SSH-опрос VPN успешен. Отдельный TCP connect на `:22` и история задержки не ведутся.
